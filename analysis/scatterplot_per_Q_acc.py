"""Plot per-question accuracy scatter plots comparing original vs rewritten questions."""
import sys, os
...
for generating_model_name in ['gpt120b']: #, 'gpt20b', 'Q235B']:
    ...
    for context_type in ['afc','orig']:
        if context_type == 'afc':
            dataset_fldrs = ['data-post-cutoff-afc','data-subset-500-afc']
        else:
            dataset_fldrs = ['data-post-cutoff', 'data-subset-500']

        for dataset_fldr in dataset_fldrs:
            ...
            if dataset_fldr.endswith('-afc') and context_type != 'afc':
                raise RuntimeError("invalid combo")
            if not dataset_fldr.endswith('-afc') and context_type == 'afc':
                raise RuntimeError("invalid combo")
