...
\begin{abstract}
	...
	Given a user question with associated answer-free grounding context, rewriting the question to reduce ambiguity produces benchmark improvements, even compared to prepending that context before the question.
...
\begin{figure}[h!]
	\vspace*{-0.5em}
	\centering
	\includegraphics[width=0.9\columnwidth]{figs/gpt120b-orig-giveaway.pdf}
	\vspace{-0.8em}
...
\begin{figure}[h!]
	%	\vspace{-1.0em}
	\centering
	\includegraphics[width=0.9\columnwidth]{figs/afc-gpt120b-orig-giveaway.pdf}
	\vspace{-0.8em}
...
A notable exception is observed in less fact-based (more reasoning intensive) benchmarks, specifically the HLE-subset and generative benchmarks. 
...
The less fact-based benchmarks (generative and HLE-subset) are also those datasets post knowledge cutoff. 
...
This effect shows up in \Cref{fig:acc_vs_embedding_gpt120b_e5-mistral-7b-instruct} with most points being in the upper-right quadrant, where improvements in benchmark accuracy correlate with improvements in cosine similarity between the question and context.
...
Another likely improvement would be a single multi-turn process which surfaces evidence, rewrites the query, and then performs another search of the corpus with the clarify query.

Additional work is required to characterize the inference time costs to this approach. 
