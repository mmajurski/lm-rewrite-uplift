...
\begin{abstract}
	...
	Given a user question with associated answer-free grounding context, rewriting the question to reduce ambiguity produces benchmark improvements always scored against the unmodified reference answer, even compared to prepending that context before the question.
	...
	Code is available at [anonymous]. %\url{https://github.com/mmajurski/grounded-synth-lm-benchmark}
\end{abstract}
...
The ongoing explosion of Language Model (LM) capability is largely a consequence of scaling laws, which have demonstrated relationships between performance and parameter count, dataset size, and computation. 
...
\begin{figure}[b!]
	...
	\caption{Rewriting questions 
		...
		using answer-free grounding context yields significant accuracy improvement (distance above line) over the original questions, as evaluated on a subset of Humanity's Last Exam (HLE). 
...
If these assumptions do not align with the user's unstated intent, the resulting answer (while potentially factually correct under that assumed interpretation) will be perceived as incorrect or irrelevant.%, or even as a "hallucination".
...
In this work, we explore the impact on system performance of adding Answer-Free Context (\texttt{AFC})---background information that is relevant to the question but which does not contain the actual answer.
...
This method can meaningfully improve accuracy on a subset of Humanity's Last Exam (HLE) (\Cref{fig:hle-uplift}).
...
\begin{mdframed}[style=spacedframe]
	...
	%	\scriptsize
	\footnotesize
	\textbf{Original question:} What kind of lasers are crystals of zinc suflde \textit{(sic)} used in? %What kind of lasers are crystals of zinc suflde used in?
...
\begin{packed_enum}
	...
	\item Analyzes the performance differential in RAG systems when retrieving direct answers versus purely auxiliary background information; and
	\item Demonstrates that this accuracy uplift necessitates a distinct rewriting phase, and cannot be replicated by simply prepending the retrieved context to a prompt. 
\end{packed_enum}
...
While LMs possess considerable general knowledge, they can struggle with domain-specific or proprietary data absent from public training sets. 
...
Dynamic context construction approaches like RAG address this by retrieving external evidence during the generation process~\citep{sharma2025retrieval}.
Grounding LM responses in external data significantly improves factual accuracy~\citep{sharma2025retrieval}.
...
Other dynamic context strategies focus on getting the most out of the context.
...
Our work demonstrates that including the disambiguation during COT is sub-optimal compared to a separate rewriting phase.
...
This work explores applying RAG query rewrite/expansion techniques directly to queries posed to LMs, which results in an accuracy uplift on evaluation benchmarks. Benchmarks routinely evaluate knowledge by posing factual questions~\citep{wang2024mmlu}. \Cref{fig:impact-of-context} shows the impact of added context on \texttt{gpt-oss-120b}'s performance (across all datasets used in this study), and demonstrates that naively adding \texttt{AFC} without rewriting the query to be clearer and less ambiguous does not improve the results as significantly as desired.
...
\begin{figure}[t!]
	...
	\includegraphics[width=0.9\columnwidth]{figs/impact_of_context_dashed-gpt-oss-120b.pdf}
	\vspace{-0.8em}
	{\small
		\caption{The quality of context presented to an LM has an impact on question-answering performance. Accuracy is highest when RAG systems correctly surface context containing the answer (\textcolor{cyan}{cyan}), but when the question is presented without context (\textcolor{orange}{orange}) or the surfaced information does not contain the answer (\textcolor{ForestGreen}{green}), performance (without query rewriting) suffers. The act of interpretation during question rewriting (\textcolor{Rhodamine}{pink}) produces an accuracy uplift beyond just prepending the \texttt{Question+Answer-Free} \texttt{Context} information used to rewrite before the question.}
...
Just prepending the \texttt{AFC} text before the question does not produce an equivalent accuracy uplift, as shown in \Cref{fig:impact-of-context}, where \texttt{Question+Answer-Free} \texttt{Context} under-performs \texttt{Rewritten} \texttt{Question}.

We hypothesize that the accuracy improvement is caused by the act of interpretation during rewriting, via clarifying and disambiguating the question separately from attempting to answer it. 
%\begin{figure}[t!]
%	\vspace*{-0.25em}
%	\centering
...
Applying rewriting to knowledge benchmarks may produce an uplift in accuracy, potentially by placing the LM under evaluation in the right ``state of mind'' to answer the question better. We explore that trend and characterize how well the effect generalizes.
% , and provides some thoughts on why it might happen. 


\subsection{Datasets \& Models}

To explore the impact of question rewriting on LM benchmark performance, datasets are needed that contain pairs of questions with associated context that provides the correct answer. While most RAG systems are designed to produce that type of information, validated reference data is in comparatively short supply, as benchmark creation across a variety of domains is historically an expensive and frequently manually intensive process. Additionally, in order to evaluate whether observed results stem from memorization/recall within the model, datasets are needed that contain information that postdates model training.
...
The originally published HLE dataset does not contain associated context with each question.
...
This work uses that validated subset of the HLE questions.
...
First is the model performing question rewriting (which is not the same as the model under evaluation).
...
In addition to rewriting the question the rewriting model must generate the correct answer, which serves as a validation check against topic drift.
...
An LM-Judge is used to extract the following properties: reformatted question similarity to the original question, reformatted answer similarity to the original answer, reformatted question ``giveaway'' score, and original question ``giveaway'' score.
...
The giveaway scores are used to remove any reformatted questions which got easier in the process of rewriting: If the reformatted giveaway score is higher than the original question (the answer is given away more in the rewritten question), that entry is discarded.
This produces a new, strictly more difficult benchmark for each of the rewriting models, with disambiguated questions.



\subsection{Evaluation methodology}

Evaluating the accuracy uplift provided by question rewriting and expansion 
...
While improving \texttt{gpt-5-mini} performance on HLE-subset is remarkable (\Cref{fig:hle-uplift}), this section evaluates the impact of query disambiguation via Answer-Free Context for all models (\Cref{tab:model_cutoff}) and datasets (\Cref{tab:dataset_publication_cutoff}) across three dimensions:
%We evaluate the impact of query disambiguation via Answer-Free Context (\texttt{AFC}) across three dimensions: 
%\begin{enumerate*}[label=(\arabic*)]
\begin{enumerate*}[label=(\alph*)]
	\item the validity of the AFC construction,
	\item the accuracy uplift provided by rewriting compared to standard RAG baselines, and 
...
\begin{figure}[h!]
	\vspace*{-0.5em}
	\centering
	\includegraphics[width=0.95\columnwidth]{figs/gpt120b-orig-giveaway.pdf}
	\vspace{-0.8em}
	\caption{Scatterplot of benchmark accuracy per-dataset and per-model showing the original question accuracy vs the original question prepended with the answer-containing context, resulting in significant over-performance.}
	\label{fig:orig-giveaway}
	\vspace{-0.5em}   
\end{figure}
To attribute performance gains to query disambiguation rather than information leakage, we first verify that our Answer-Free Context does not inadvertently contain the answer to the benchmark question. 
\Cref{fig:orig-giveaway} illustrates model benchmark performance when the correct answer is present in the context. 
As expected, prepending answer-containing context yields a significant improvement in accuracy.
This confirms that models successfully utilize answer information when present within the context.

In contrast, \Cref{fig:orig-giveaway-afc} plots model performance when the \texttt{Answer-Free Context} is prepended to the original question. 
The absence of consistent model over-performance lends evidence that the AFC creation process successfully removed the direct answer. 
Consequently, any accuracy uplift observed in subsequent experiments can be attributed to query disambiguation rather than rote extraction of the answer.
\begin{figure}[h!]
	%	\vspace{-1.0em}
	\centering
	\includegraphics[width=0.95\columnwidth]{figs/afc-gpt120b-orig-giveaway.pdf}
	\vspace{-0.8em}
	\caption{Scatterplot of benchmark accuracy per-dataset and per-model showing the original question accuracy vs the original question prepended with the answer-free context, resulting in no trend of accuracy improvement attributable to the context.}
...
\subsection{Accuracy Uplift from Query Rewriting} 
...
\Cref{fig:r_minus_q} presents the distribution of accuracy improvement measured as ($\texttt{Rewrite\_Q} - \texttt{Orig\_Q}$) across all models and datasets. 
...
\Cref{fig:gpt120b-afc} further breaks this down by dataset, showing that most (model,dataset) combinations fall above the identity line ($y=x$), confirming that rewriting rarely degrades performance relative to the original query (at the benchmark level), and never degrades performance for any dataset pre model knowledge cutoff.
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
	\includegraphics[width=0.95\columnwidth]{figs/gpt120b-afc.pdf}
	\vspace{-0.8em}
...
Considering the rewritten questions likely contain more detailed information drawn from context, comparing them directly against the original is not a fair comparison. 
A stricter evaluation compares the Rewritten Question against a standard RAG approach where the model receives the original question with the context prepended ($\texttt{Orig\_Q+AFC}$).
...
\begin{figure}[h!]
	...
	\vspace{-0.8em}
	\caption{Per-dataset per-model difference in benchmark accuracy between the rewritten questions and the original questions with associated answer-free context. The violin plot distribution highlights the range of accuracy deltas over all datasets for each model evaluated.}
...
\paragraph{The Reasoning Gap}  % Also term it something like post-data cutoff. We can disambiguate between this being an effect of datasets in the training population, or whether teh generative and HLE benchmarks are less fact-heavy.
A notable exception is observed in less direct fact-recall (more reasoning intensive) benchmarks, specifically the HLE-subset and generative benchmarks. 
...
While rewriting improves over the isolated \texttt{Orig\_Q} query, more complex reasoning tasks benefit more from having the full raw context available in the window during inference than from a condensed rewritten query. 
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
	\includegraphics[width=0.9\columnwidth]{figs/acc_uplift_gpt120/rafc_minus_qafc_giveaway_dataset.pdf}
	\vspace{-0.8em}
	\caption{Per-dataset per-model difference in benchmark accuracy between the rewritten questions and the original questions with associated answer-free context. The violin plot distribution highlights the range of accuracy deltas over all models for each dataset evaluated.}
	\label{fig:r_minus_q_afc_giveaway_dataset}
	\vspace{-1em}   
\end{figure}

In \Cref{fig:r_minus_q_afc_giveaway_dataset} the first 5 datasets are from post knowledge cutoff. 
Additionally, due to a paucity of datasets released since the beginning of the year, all but the HLE-subset were generatively created benchmarks using either~\citep{majurski2025generative} or~\citep{shashidhar2025yourbench}.
...
The less direct fact-recall type benchmarks (generative and HLE-subset) are also those datasets post knowledge cutoff for all models (see \Cref{tab:dataset_publication_cutoff} and \Cref{tab:model_cutoff}. 
So this lack of improvement from rewriting might stem from a model data memorization effect, or from the questions requiring more reasoning and less direct recall.
...
However, combining both strategies limits the potential downsides to rewriting. %yields the highest performance. 
\Cref{fig:rafc_giveaway_minus_qafc_giveaway_dataset} demonstrates that \texttt{Rewrite\_Q+AFC} limits the potential accuracy upside but reduces the number of datasets which show no average improvement in accuracy.
This might indicate that disambiguation and context inclusion are complementary.
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
	\includegraphics[width=0.9\columnwidth]{figs/acc_uplift_gpt120/rafc_giveaway_minus_qafc_giveaway_dataset.pdf}
	\vspace{-0.8em}
	\caption{Per-dataset improvement in benchmark accuracy from the Rewrite\_Q with AFC compared to the Orig\_Q with AFC during benchmark evaluation.}
...
\subsection{Mechanism of Action: Alignment and Task Separation} 
...
To understand the source of the accuracy uplift, we analyze the semantic alignment between queries and context.
...
We observe a positive correlation: rewritten questions systematically exhibit higher cosine similarity to the grounding context, and larger increases in similarity correlate with larger accuracy gains. 
This effect shows up in \Cref{fig:acc_vs_embedding_gpt120b_e5-mistral-7b-instruct} with most points ($91\%$ or 222 out of 243) being in the upper-right quadrant, where improvements in benchmark accuracy correlate with improvements in cosine similarity between the question and context.
...
We implemented an ``In-Situ'' baseline where the model is prompted to rewrite the question internally before answering. 
\Cref{fig:insitu_rafc_minus_q_afc_giveaway_dataset} shows that the accuracy uplift disappears under this paradigm. 
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
...
CoT rewriting was evaluated using the same prompt only lightly modified to have the LM under evaluation first rewrite for disambiguation before answering the rewritten question. 
...
Evaluations into the impact of question rewriting herein rely heavily on extractive QA datasets which have pairs of question and context.
The exception is HLE-subset where the questions are post-facto grounded using internet search~\citep{HleFutureHouse}.
...
This limitation is a natural consequence of the availability of public dataset containing questions paired with grounding context.

\section{Conclusion}

This work demonstrates a novel utilization of the information dynamic context retrieval systems surface for a query.
RAG utility extends beyond a binary success or failure of surfacing the direct answer.
We introduce and validate query disambiguation leveraging this potentially answer-free context information. 
This demonstrates that surfaced background information can be leveraged to enhance LM performance even when it does not contain the answer. 
By using \texttt{AFC} to rewrite and disambiguate user queries prior to inference, we observed substantial accuracy uplifts across multiple benchmarks, most notably doubling the performance of \texttt{gpt-5-mini} on a subset of Humanity’s Last Exam (HLE).

Our analysis produces three insights for the design of future dynamic context grounded systems. 
First, the mechanism of rewrite improvement is measurable and predictable because accuracy gains correlate with increased semantic alignment (cosine similarity) between the rewritten query and the grounding context. 
Second, we identify two behaviors in response to query rewriting; one cluster where primarily factual queries benefit maximally from rewriting alone, while more complex questions (such as HLE) achieve peak performance when the model is provided with both the disambiguated query and the raw context. 
Finally, we establish the necessity of task separation. 
The inability to replicate the rewrite performance improvement during an in-situ Chain-of-Thought indicates that query refinement and answer generation compete for cognitive resources or context attention.
...
This work suggests treating dynamic context construction methodologies as not purely as evidence collectors, but a collaborative partner in query formulation.
Aligning the users query to the document corpus and clarifying the requested information allows LMs to more precisely and correctly provide the information a user actually wants. 



\subsection{Future work}
This study only used single-shot query rewriting.
Accuracy improvements are likely possible by extending this to a best of N approach which attempts to disambiguate the users questions in multiple directions based on surfaced evidence, before asking the user which direction actually aligns with their intent. 
Another likely improvement would be a single multi-turn process which surfaces evidence, rewrites the query, and then performs another search of the corpus with the clarified query.

Additional work is required to characterize the additional inference time costs to this approach. 
How many additional tokens are being spent and RAG database calls made to perform query disambiguation.
Or how small an LM can be used for this process while still maintaining result fidelity? 
This methodology provides another test time compute knob that can be turned up or down depending on the requirements of the situation. 
