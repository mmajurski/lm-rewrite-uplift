...
While improving \texttt{gpt-5-mini} performance on HLE-subset from 13.9\% to ~37.2\% is remarkable (\Cref{fig:hle-uplift}), this section evalautes the impact of query disambiguation via Answer-Free Context for all models (\Cref{tab:model_cutoff}) and datasets (\Cref{tab:dataset_publication_cutoff}) across three dimensions:
...
As expected, prepending answer-containing context yields a signifcant improvement in accuracy, confirming that models successfully utilize explicit answers when present within the context.
\begin{figure}[h!]
	\vspace*{-0.5em}
	\centering
...
The absence of consistenct model over-performacne lends evidence that the AFC generation process successfully removed the direct answer. 
...
This demonstrates the rewritten questions produce broadly improved benchmark performance for all models compared to the original questions.
Considering that the rewritten questions likely contain more detailed information drawn from context, this not a fair comparison. 
\Cref{fig:gpt120b-afc} further breaks this down by dataset, showing that all model-dataset combinations fall above the identity line ($y=x$), confirming that rewriting never degrades performance relative to the original query (at the benchmark level).
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
...
Additionally, due to a paucity of datasets released since the beginning of the year, all but the HLE-subset were generativly created benchmarks using either~\citep{majurski2025generative} or~\citep{shashidhar2025yourbench}.
...
Unfortunatly, the datasets available for this study have two conflated effects.
The less fact-based benchmarks (generative and HLE-subset) are also those datasets post knowledge cutoff. 
So this lack of improvement from rewriting might stem from a model data memorization effect, or from the questions being less fact-based.
...
\Cref{fig:rafc_giveaway_minus_qafc_giveaway_dataset} demonstrates that \texttt{Rewrite\_Q+AFC} strongly outperforms \texttt{Orig\_Q+AFC}, particularly on the Fermi estimation dataset, confirming that disambiguation and context inclusion are complementary.
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
...
\subsection{Mechanism of Action: Alignment and Task Separation} To understand the source of the accuracy uplift, we analyze the semantic alignment between queries and context. \Cref{fig:acc_vs_embedding_gpt120b_e5-mistral-7b-instruct} plots the change in benchmark accuracy against the change in cosine similarity (embedded via \texttt{e5-mistral}) between the question and the context. We observe a positive correlation: rewritten questions systematically exhibit higher cosine similarity to the grounding context, and larger increases in similarity correlate with larger accuracy gains. This suggests the rewriting process effectively "primes" the query to align with the latent space of the relevant information.

Finally, we test whether this rewriting must be an explicit preprocessing step or if it can be induced via Chain-of-Thought (CoT). We implemented an "In-Situ" baseline where the model is prompted to rewrite the question internally before answering. \Cref{fig:insitu_rafc_minus_q_afc_giveaway_dataset} shows that the accuracy uplift disappears under this paradigm. This result highlights the necessity of distinct task separation: the cognitive load or context window dynamics of rewriting \textit{and} answering in a single pass negates the benefits of disambiguation.
