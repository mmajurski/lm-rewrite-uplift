...
\begin{figure}[b!]
	...
	\caption{Rewriting questions 
		...
		using answer-free grounding context yields significant accuracy improvement (distance above line) over the original questions, as evaluated on a subset of Humanity's Last Exam (HLE). See \Cref{fig:all-scatterplots} for complete legend.
...
In professional and enterprise workflows, system performance depends on (RAG) to dynamically construct context for the LM based upon user queries~\citep{lewis2020retrieval,NEURIPS2024_db93ccb6}.
...
\begin{figure}[t!]
	...
	\vspace{-0.8em}
	{\small
		\caption{The quality of context presented to an LM has an impact on question-answering performance. Accuracy is highest when RAG systems surface context containing the answer (\textcolor{cyan}{cyan}), but when the question is presented without context (\textcolor{orange}{orange}) or the surfaced information does not contain the answer (\textcolor{ForestGreen}{green}), performance (without query rewriting) suffers. The act of interpretation during question rewriting (\textcolor{Rhodamine}{pink}) produces an accuracy improvement beyond just prepending the \texttt{Question + Answer-Free} \texttt{Context} information used to rewrite before the question.}
...
\begin{figure*}[t!b]
	...
	\caption{Performance of various LMs with and without answer-containing context and answer-free context. The \textit{x}-axis shows the original question benchmark accuracy while the \textit{y}-axis shows the rewritten question benchmark accuracy; distance above the line conveys improvement. Colors represent the different models tested (\cref{tab:model_cutoff}) and shapes represent the dataset (\cref{tab:dataset_publication_cutoff}). (top left) \textit{Answers are present in the context} (control case): Performance with the addition of context that contains the answer. (top right) \textit{Answers not present in the context}: Simply providing context without question rewriting does not improve performance. (bottom left) \textit{Rewritten question plus answer-free context}: Improvement in accuracy caused by rewriting the question and adding \texttt{AFC} (our proposed approach).}
	\label{fig:all-scatterplots}
\end{figure*}

To characterize one hypothesis for why this effect shows up, we leverage an embedding model (\texttt{e5-mistral-7b-instruct}) to understand the change in cosine distance between the original question and the context compared to the cosine distance between the rewritten question and the context.
...
% \begin{figure*}[t!b]
	...
	%     \subfloat{\includegraphics[width=0.45\textwidth]{figs/gpt120b-afc.pdf}}
	%     \subfloat{\includegraphics[width=0.45\textwidth]{figs/LEGEND.png}}
	%     \caption{Performance of various LMs with and without answer-containing context and answer-free context. The \textit{x}-axis shows the original question benchmark accuracy while the \textit{y}-axis shows the rewritten question benchmark accuracy; distance above the line conveys improvement. Colors represent the different models tested (\cref{tab:model_cutoff}) and shapes represent the dataset (\cref{tab:dataset_publication_cutoff}). (left) \textit{Answers are present in the context} (control case): Performance with the addition of context that contains the answer. (center) \textit{Answers not present in the context}: Simply providing context without question rewriting does not improve performance. (right) \textit{Rewritten question plus answer-free context}: Improvement in accuracy caused by rewriting the question and adding \texttt{AFC} (our proposed approach).}
	%     \label{fig:all-scatterplots}
	% \end{figure*}
...
% \begin{figure}[h!]
	...
	% 	\vspace{-0.8em}
	% 	\caption{\textit{Answers are present in the context.} Benchmark accuracy per dataset and per model, showing the original question accuracy \textit{vs.} performance with the addition of context that contains the answer, resulting in performance improvement.}
...
\Cref{fig:all-scatterplots} (left) illustrates performance when the correct answer \textit{is present} in the context. As expected, prepending answers yields a significant improvement in accuracy. This confirms that models successfully utilize answer information when it is present. In contrast, \Cref{fig:all-scatterplots} (center) plots model performance when \texttt{Answer-Free Context} is prepended to the original question. The absence of consistent performance improvement shows that the AFC creation process successfully removed the direct answer.  
...
% \begin{figure}[h!]
	...
	% 	\vspace{-0.8em}
	% 	\caption{\textit{Answers not present in the context.} Benchmark accuracy per-dataset and per-model showing accuracy on the original question \textit{vs.} the original question prepended with answer-free context. No accuracy improvement is attributable solely to context.}
...
\begin{figure}[h!]
	...
	\vspace{-0.8em}
	\caption{Per-dataset per-model difference in benchmark accuracy between the rewritten question and the original question. The violin plot distribution highlights the range of accuracy deltas over all datasets for each model evaluated. Average accuracy improvement of 0.1303.}
	\label{fig:r_minus_q}
	\vspace{-1em}   
\end{figure}
...
\Cref{fig:all-scatterplots} (right) further breaks this down by dataset, showing that most (model,dataset) combinations fall above the identity line ($y=x$), confirming that rewriting rarely degrades performance relative to the original query (at the benchmark level), and never degrades performance for any dataset pre-model knowledge cutoff.
% \begin{figure}[h!]
	% 	\vspace{-0.5em}
	% 	\centering
	% 	\includegraphics[width=0.95\columnwidth]{figs/gpt120b-afc.pdf}
	% 	\vspace{-0.8em}
	% 	\caption{\textit{Rewritten question plus answer-free context.} Scatterplot showing the per-dataset per-model improvement in benchmark accuracy caused by rewriting the question. The \textit{x}-axis shows the original question benchmark accuracy while the \textit{y}-axis shows the rewritten question benchmark accuracy.}
...
In this case, the model under evaluation has access to all information the rewriting model did. 
...
It is possible that the lack of improvement from rewriting may stem from a data memorization effect or from the fact that the questions require more reasoning and less direct recall.
...
% 
Combining both strategies limits the potential downsides to rewriting. %yields the highest performance. 
\Cref{fig:rafc_giveaway_minus_qafc_giveaway_dataset} demonstrates that \texttt{Rewrite\_Q+AFC} limits the potential accuracy improvement but reduces the number of datasets which show no average improvement in accuracy.
...
\begin{figure}[h!]
	...
	\vspace{-0.8em}
	\caption{Per-dataset improvement in benchmark accuracy from \texttt{Rewrite\_Q} with \texttt{AFC} compared to the \texttt{Orig\_Q} with \texttt{AFC} during benchmark evaluation. Average accuracy improvement of 0.0875.}
...
This effect is shown in \Cref{fig:acc_vs_embedding_gpt120b_e5-mistral-7b-instruct} with most points ($91\%$ or 222 out of 243) being in the upper-right quadrant, where improvements in benchmark accuracy correlate with improvements in cosine similarity between the question and the context---that is, rewritten questions that have a higher cosine similarities to the context also have a higher benchmark accuracy.

It is worth noting that every model and dataset combination drawn from before the knowledge cutoff appear in the upper right quadrant.
...
The exception is HLE-subset, where the questions are \textit{post facto} grounded using internet search~\citep{HleFutureHouse}.
...
This limitation is an inevitable consequence of the availability of public datasets containing questions paired with grounding context.
