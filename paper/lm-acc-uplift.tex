...
%TODO The "Giveaway" Terminology:
%Issue: The term "Giveaway" is colloquial and slightly confusing in the axis labels and text.
%Fix: Standardize on "Answer-Containing Context" (ACC) and "Answer-Free Context" (AFC). In plots, use labels like Orig + ACC vs Orig + AFC.
...
\Cref{fig:gpt120b-afc} further breaks this down by dataset, showing that all (model,dataset) combinations fall above the identity line ($y=x$), confirming that rewriting never degrades performance relative to the original query (at the benchmark level) for datasets before the knowledge cutoff.
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
...
A notable exception is observed in less fact-based benchmarks, specifically the HLE-subset and generative benchmarks. 
...
While rewriting improves over the bare \texttt{Orig\_Q} query, more complex reasoning tasks benefit more from having the full raw context available in the window during inference than from a condensed rewritten query. 
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
...
So this lack of improvement from rewriting might stem from a model data memorization effect, or from the questions requiring more reasoning and less direct recall. % being less fact-based.
...
However, combining both strategies yields the highest performance. 
\Cref{fig:rafc_giveaway_minus_qafc_giveaway_dataset} demonstrates that \texttt{Rewrite\_Q+AFC} outperforms \texttt{Orig\_Q+AFC}, particularly on the Fermi estimation dataset, confirming that disambiguation and context inclusion are complementary.
\begin{figure}[h!]
	\vspace{-0.5em}
	\centering
...
\Cref{fig:acc_vs_embedding_gpt120b_e5-mistral-7b-instruct} plots the change in benchmark accuracy against the change in cosine similarity (embedded via \texttt{e5-mistral}) between the question and the context. 
...
This was evaluated using the same question rewriting prompt lightly modified to have the LM under evaluation first rewrite for disambiguation before answering the rewritten question. 
