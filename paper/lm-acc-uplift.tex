...
We demonstrate that even when RAG fails to surface a direct answer, the retrieved background information can be used to perform a contextually grounded disambiguation rewrite (\Cref{fig:figure1}) to improve an underspecified query before it is answered.
...
CoT-type reasoning approaches enable the LM to employ self-directed question rewriting during the process of figuring out how best to respond; \textcolor{red}{Our work differs from this in that...}
% TODO respond to the above comment
...
\begin{figure}[t!]
	\vspace*{-0.25em}
	\centering
	\includegraphics[width=0.8\columnwidth]{figs/impact_of_context_gpt-oss-120b.pdf}
	\vspace{-0.8em}
	{\small
		\caption{The quality of context presented to an LM has an impact on question-answering performance. Accuracy is high when RAG systems correctly surface context containing the answer (\textcolor{green}{green}), but when the question is presented without context (\textcolor{red}{red}) or the surfaced information does not contain the answer (\textcolor{blue}{blue}), performance (without query rewriting) suffers. The act of interpretation during question rewriting (\textcolor{cyan}{cyan}) produces an accuracy uplift beyond just prepending the \texttt{Question+Answer-Free} \texttt{Context} information used to rewrite before the question.}
