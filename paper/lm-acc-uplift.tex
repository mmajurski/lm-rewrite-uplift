...
This work explores the interplay between the quality of the background grounding information in the LM context window and the quality of the resulting answer.
...
This accuracy uplift cannot be fully recovered just through prompting at question evaluation time, separate rewriting and answering phases are required.
...
This capability growth has opened a widening chasm between what modern LMs can demonstrably do and how we measure the spiky model intelligence. % abilities.
...
This leads them to omit critical background and related information when formulating a query, believing it to be self-evident.
...
Our methodology evaluates the impact of the retrieved informational content on LM+RAG system performance; both when the correct answer is surfaced and when only related background information is found.
...
Additionally, we demonstrate benchmark accuracy improvement using a contextually grounded question disambiguation rewrite.
...
This rewrite works with Answer-Free Context (\texttt{AFC})---background information that is relevant to the question but which does not contain the answer. 
...
The core of our approach is shown in \Cref{fig:figure1}.
...
%First, an LM generates Answer-Free Context for a particular question; then another LM (which may be a different model) is asked to rewrite the question using the original question plus that context. 
...
% TODO fill in and finalize the contributions
\noindent The contributions of this work are as follows.
\begin{packed_enum}
	\item We explore the impact of RAG surfacing the correct answer vs only background information on document grounded LM accuracy;
	\item We demonstrate leveraging answer-free background context for question disambiguation to improve model accuracy; and
%	\item We compare and contrast simply pre-pending surfaced dynamic context against using that information for question disambiguation, demonstrating that question rewriting out-performs context pre-pending.
	\item We show that using surfaced context for question disambiguation and rewriting out-performs pre-pending that information before the question.
\end{packed_enum}
...
To bridge this lexical gap, modern RAG systems, like their classic predecessors, employ query expansion and rewriting \citep{jagerman2023query,zhou2023unified}. 
...
LMs have proven adept at this rewriting, expansion, and contextualization task; disambiguating queries using carefully designed prompts \citep{wilson2025contextualizing}. 
...
Frameworks like AGREE formulate methods to have the LM cite its sources within the context window (drawn from grounding documents) \citep{ye2023effective}.
...
Any CoT type reasoning approaches enable the LM to employ some form of question rewriting during the process of figuring out how best to respond.
...
This work explores applying a RAG query rewrite/expansion to produce accuracy uplift on LM evaluation benchmarks. %on direct factual questions posed to LMs.
Benchmarks like MMLU-pro evaluate knowledge by posing factual questions to models~\citep{wang2024mmlu}.
...
Queries without any supporting context produce mixed results (blue); \texttt{Question+Context}, in which the context contains the desired answer, performs well (red); and \texttt{Question+AFC} does not match the context-supported accuracy (green).
\begin{figure}[t!]
	\vspace*{-1.5em}
	\centering
...
\begin{figure}[t!]
	...
	\vspace{-1.0em}   
\end{figure}
However, the question rewriting using \texttt{AFC} does not recover the full performance improvement of surfacing the grounding context which directly contains the correct answer.
...
The originally published HLE dataset does not contains associated context with each question, however FutureHouse released manual validation for a subset of chemistry/biology questions with grounding literature~\citep{HleFutureHouse}.
