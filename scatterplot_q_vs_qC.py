...
if os.path.exists(f'avg_uplift_per_model_dataset.json'):
    with open(f'avg_uplift_per_model_dataset.json', 'r') as f:
        avg_uplift_per_model_dataset = json.load(f)
else:
    ...
    with open(f'avg_uplift_per_model_dataset.json', 'w') as f:
...
for m_idx, model_name in enumerate(all_models):            
    ...
    plt.title('Impact of Context Information Content \non Benchmark Accuracy')
