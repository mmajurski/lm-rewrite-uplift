...
@task
def pubmed_qa(dataset_fldr, question_key):
    return base_task(dataset_fldr, question_key)
...
if __name__ == '__main__':
    ...
    config = GenerateConfig(max_connections=args.batch_size, max_tokens=4096)
    models = list()
    ...
    models_dict['google/gemma-3-4b-it'] = get_model(model="v_llm/google/gemma-3-4b-it", base_url="https://pn120393.nist.gov:8443/v1", config=config, api_key=os.getenv("VLLM_API_KEY"))
    ...
    #models_dict['openai/gpt-oss-20b'] = get_model(model="v_llm/openai/gpt-oss-20b", base_url="https://pn125916.nist.gov:8444/v1", config=config, api_key=os.getenv("VLLM_API_KEY"))
    
    models_dict['microsoft/phi-4'] = get_model(model="v_llm/microsoft/phi-4", base_url="https://pn125916.nist.gov:8445/v1", config=config, api_key=os.getenv("VLLM_API_KEY"))
    models_dict['Qwen/Qwen2.5-7B-Instruct'] = get_model(model="v_llm/Qwen/Qwen2.5-7B-Instruct", base_url="https://pn125916.nist.gov:8446/v1", config=config, api_key=os.getenv("VLLM_API_KEY"))


    #models_dict['meta-llama/Llama-3.3-70B-Instruct'] = get_model(model="v_llm/meta-llama/Llama-3.3-70B-Instruct", base_url="https://pn125918.nist.gov:8443/v1", config=config, api_key=os.getenv("VLLM_API_KEY"))
